...
import { AddressTypeNotMatchError, InvalidAddressVersionError, MixedCaseAddressError, } from '../../errors/address.js';
...
export function decodeBase32Address({ address, strict = true, }) {
    ...
    const [version, ...addressBytes] = convertBit(new Uint8Array([...payload5Bits]), 5, 8);
    ...
    if (strict) {
        const valid = polyMod(new Uint8Array([...netName5Bits, 0, ...payload5Bits, ...checksum5Bits]));
        if (valid) {
            throw new InvalidAddressVersionError({ address });
        }
    }
