...
function encodeParams(preparedParams) {
    ...
    for (let i = 0; i < preparedParams.length; i++) {
        ...
        if (dynamic)
            ...
            staticSize += (0, viem_1.size)(encoded);
    }
    const staticParams = [];
    const dynamicParams = [];
    let dynamicSize = 0;
    for (let i = 0; i < preparedParams.length; i++) {
        ...
        if (dynamic) {
            ...
            dynamicSize += (0, viem_1.size)(encoded);
        }
        else {
            staticParams.push(encoded);
        }
    }
    return (0, viem_1.concat)([...staticParams, ...dynamicParams]);
}
function encodeAddress(value) {
    if (!(0, isAddress_js_1.isAddress)(value))
        throw new address_js_1.InvalidAddressError({ address: value });
    const hexAddress = (0, base32AddressToHex_js_1.base32AddressToHex)({ address: value });
    return { dynamic: false, encoded: (0, viem_1.padHex)(hexAddress) };
}
function encodeArray(value, { length, param, }) {
    ...
    if (dynamic || dynamicChild) {
        ...
        if (dynamic) {
            ...
            return {
                dynamic: true,
                encoded: preparedParams.length > 0 ? (0, viem_1.concat)([length, data]) : length,
            };
        }
        if (dynamicChild)
    ...
    return {
        dynamic: false,
        encoded: (0, viem_1.concat)(preparedParams.map(({ encoded }) => encoded)),
    };
}
function encodeBytes(value, { param }) {
    ...
    if (!paramSize) {
        ...
        return {
            dynamic: true,
            encoded: (0, viem_1.concat)([(0, viem_1.padHex)((0, viem_1.numberToHex)(bytesSize, { size: 32 })), value_]),
        };
    }
    ...
    return { dynamic: false, encoded: (0, viem_1.padHex)(value, { dir: 'right' }) };
}
function encodeBool(value) {
    if (typeof value !== 'boolean')
        throw new viem_1.BaseError(`Invalid boolean value: "${value}" (type: ${typeof value}). Expected: \`true\` or \`false\`.`);
    return { dynamic: false, encoded: (0, viem_1.padHex)((0, viem_1.boolToHex)(value)) };
}
function encodeNumber(value, { signed }) {
    return {
        dynamic: false,
        encoded: (0, viem_1.numberToHex)(value, {
            size: 32,
            signed,
        }),
    };
...
function encodeString(value) {
    ...
    return {
        dynamic: true,
        encoded: (0, viem_1.concat)([
            (0, viem_1.padHex)((0, viem_1.numberToHex)((0, viem_1.size)(hexValue), { size: 32 })),
            ...parts,
        ]),
    };
}
function encodeTuple(value, { param }) {
    ...
    return {
        ...
        encoded: dynamic
            ...
            : (0, viem_1.concat)(preparedParams.map(({ encoded }) => encoded)),
    };
}
