...
var compiledMulticall = json.parse(JSON.stringify(Multicall_default));
var compiledJediswapPair = json.parse(JSON.stringify(JediswapPair_default));
var compiledEkuboPositions = json.parse(JSON.stringify(EkuboPositions_default));

// src/constants/safety.ts
import { Fraction, Percent as Percent2 } from "@uniswap/sdk-core";
...
var TEAM_ALLOCATION_SAFETY_BOUNDS = {
  [0 /* SAFE */]: new Percent2(1, 100),
  // 1%
  [1 /* CORRECT */]: new Percent2(10, 100)
  // 10%
};
var LIQUIDITY_LOCK_SAFETY_BOUNDS = {
  [0 /* SAFE */]: moment.duration(100, "years"),
  [1 /* CORRECT */]: moment.duration(3, "months")
};
...
import { constants as constants2, getChecksumAddress } from "starknet";
...
var Ether = {
  [constants2.StarknetChainId.SN_GOERLI]: {
    address: ETH_ADDRESSES[constants2.StarknetChainId.SN_GOERLI],
    symbol: "ETH" /* ETH */,
    name: "Ether",
    decimals: 18,
    camelCased: true,
    usdcPair: {
      address: JEDISWAP_ETH_USDC[constants2.StarknetChainId.SN_GOERLI],
      reversed: true
    }
  },
  [constants2.StarknetChainId.SN_MAIN]: {
    address: ETH_ADDRESSES[constants2.StarknetChainId.SN_MAIN],
    symbol: "ETH" /* ETH */,
    name: "Ether",
    decimals: 18,
    camelCased: true,
    usdcPair: {
      address: JEDISWAP_ETH_USDC[constants2.StarknetChainId.SN_MAIN],
      reversed: false
    }
  }
};
var Stark = {
  [constants2.StarknetChainId.SN_GOERLI]: {
    address: STRK_ADDRESSES[constants2.StarknetChainId.SN_GOERLI],
    symbol: "STRK" /* STRK */,
    name: "Stark",
    decimals: 18,
    camelCased: true,
    usdcPair: {
      address: JEDISWAP_STRK_USDC[constants2.StarknetChainId.SN_GOERLI],
      reversed: true
    }
  },
  [constants2.StarknetChainId.SN_MAIN]: {
    address: STRK_ADDRESSES[constants2.StarknetChainId.SN_MAIN],
    symbol: "STRK" /* STRK */,
    name: "Stark",
    decimals: 18,
    camelCased: true,
    usdcPair: {
      address: JEDISWAP_STRK_USDC[constants2.StarknetChainId.SN_MAIN],
      reversed: false
    }
  }
};
var USDCoin = {
  [constants2.StarknetChainId.SN_GOERLI]: {
    address: USDC_ADDRESSES[constants2.StarknetChainId.SN_GOERLI],
    symbol: "USDC" /* USDC */,
    name: "USD Coin",
    decimals: 6,
    camelCased: true
  ...
  [constants2.StarknetChainId.SN_MAIN]: {
    address: USDC_ADDRESSES[constants2.StarknetChainId.SN_MAIN],
    symbol: "USDC" /* USDC */,
    name: "USD Coin",
    decimals: 6,
    camelCased: true
...
var QUOTE_TOKENS = {
  [constants2.StarknetChainId.SN_GOERLI]: {
    [getChecksumAddress(ETH_ADDRESSES[constants2.StarknetChainId.SN_GOERLI])]: Ether[constants2.StarknetChainId.SN_GOERLI],
    [getChecksumAddress(STRK_ADDRESSES[constants2.StarknetChainId.SN_GOERLI])]: Stark[constants2.StarknetChainId.SN_GOERLI],
    [getChecksumAddress(USDC_ADDRESSES[constants2.StarknetChainId.SN_GOERLI])]: USDCoin[constants2.StarknetChainId.SN_GOERLI]
  },
  [constants2.StarknetChainId.SN_MAIN]: {
    [getChecksumAddress(ETH_ADDRESSES[constants2.StarknetChainId.SN_MAIN])]: Ether[constants2.StarknetChainId.SN_MAIN],
    [getChecksumAddress(STRK_ADDRESSES[constants2.StarknetChainId.SN_MAIN])]: Stark[constants2.StarknetChainId.SN_MAIN],
    [getChecksumAddress(USDC_ADDRESSES[constants2.StarknetChainId.SN_MAIN])]: USDCoin[constants2.StarknetChainId.SN_MAIN]
  }
};
var DEFAULT_QUOTE_TOKEN_ADDRESSES = {
  [constants2.StarknetChainId.SN_GOERLI]: getChecksumAddress(ETH_ADDRESSES[constants2.StarknetChainId.SN_GOERLI]),
  [constants2.StarknetChainId.SN_MAIN]: getChecksumAddress(ETH_ADDRESSES[constants2.StarknetChainId.SN_MAIN])
};

// src/factory/default.ts
import { Fraction as Fraction3, Percent as Percent3 } from "@uniswap/sdk-core";
import { CallData as CallData2, getChecksumAddress as getChecksumAddress2, hash as hash2, shortString, stark, uint256 as uint2562 } from "starknet";
...
var Factory = class {
  ...
  async getBaseMemecoin(address) {
    ...
    return {
      ...
      decimals: DECIMALS,
      totalSupply: uint2562.uint256ToBN({ low: totalSupply[0], high: totalSupply[1] }).toString()
    };
  }
  //
  // GET LAUNCH
  ...
  async getMemecoinLaunchData(address) {
    ...
    switch (liquidityType) {
      ...
      case "JEDISWAP_ERC20" /* JEDISWAP_ERC20 */: {
        const baseLiquidity = {
          ...
          quoteAmount: uint2562.uint256ToBN({ low: launchParams[3], high: launchParams[4] }).toString()
        };
        liquidity = {
          ...baseLiquidity,
    ...
    return {
      ...
      launch: {
        teamAllocation: uint2562.uint256ToBN({ low: teamAllocation[0], high: teamAllocation[1] }).toString(),
  ...
  async getEkuboFees(memecoin) {
    ...
    const calldata = CallData2.compile([
    ...
    return new Fraction3(
      (new Fraction3(memecoin.address).lessThan(memecoin.quoteToken.address) ? fees1 : fees0).toString(),
  ...
  getCollectEkuboFeesCalldata(memecoin) {
    ...
    const collectFeesCalldata = CallData2.compile([
      memecoin.liquidity.ekuboId,
      // ekubo pool id
  ...
  getExtendLiquidityLockCalldata(memecoin, seconds) {
    if (!memecoin?.isLaunched || memecoin.liquidity.type === "EKUBO_NFT" /* EKUBO_NFT */) return;
    const extendCalldata = CallData2.compile([
  ...
  getDeployCalldata(data) {
    ...
    const constructorCalldata = CallData2.compile([
      ...
      data.name,
      data.symbol,
      uint2562.bnToUint256(BigInt(data.initialSupply) * BigInt(decimalsScale(DECIMALS))),
      salt
    ]);
    const tokenAddress = hash2.calculateContractAddressFromHash(
      salt,
  ...
  async getEkuboLaunchCalldata(memecoin, data) {
    ...
    const teamAllocationFraction = data.teamAllocations.reduce((acc, { amount }) => acc.add(amount), new Fraction3(0));
    const teamAllocationPercentage = new Percent3(
      teamAllocationFraction.quotient,
      new Fraction3(memecoin.totalSupply, decimalsScale(DECIMALS)).quotient
    );
    const teamAllocationQuoteAmount = new Fraction3(data.startingMarketCap).divide(quoteTokenPrice).multiply(teamAllocationPercentage.multiply(data.fees.add(1)));
    const uin256TeamAllocationQuoteAmount = uint2562.bnToUint256(
      BigInt(teamAllocationQuoteAmount.multiply(decimalsScale(data.quoteToken.decimals)).quotient.toString())
    );
    const initialPrice = +new Fraction3(data.startingMarketCap).divide(quoteTokenPrice).multiply(decimalsScale(DECIMALS)).divide(new Fraction3(memecoin.totalSupply)).toFixed(DECIMALS);
    ...
    const transferCalldata = CallData2.compile([
      FACTORY_ADDRESSES[this.config.chainId],
      // recipient
    ...
    const initialHoldersAmounts = data.teamAllocations.map(
      ({ amount }) => uint2562.bnToUint256(BigInt(amount) * BigInt(decimalsScale(DECIMALS)))
    );
    const launchCalldata = CallData2.compile([
      memecoin.address,
      // memecoin address
      data.antiBotPeriod,
  ...
  async getStandardAMMLaunchCalldata(memecoin, data) {
    ...
    const quoteAmount = new Fraction3(data.startingMarketCap).divide(quoteTokenPrice).multiply(new Fraction3(1).subtract(teamAllocationPercentage));
    const uin256QuoteAmount = uint2562.bnToUint256(BigInt(quoteAmount.multiply(decimalsScale(18)).quotient.toString()));
    const initialHolders = data.teamAllocations.map(({ address }) => address);
    const initialHoldersAmounts = data.teamAllocations.map(
      ({ amount }) => uint2562.bnToUint256(BigInt(amount) * BigInt(decimalsScale(DECIMALS)))
    );
    const approveCalldata = CallData2.compile([
      FACTORY_ADDRESSES[this.config.chainId],
      // spender
      uin256QuoteAmount
    ]);
...
import { Fraction as Fraction4 } from "@uniswap/sdk-core";
function getStartingMarketCap(memecoin, quoteTokenPriceAtLaunch) {
  ...
  switch (memecoin.liquidity.type) {
    case "STARKDEFI_ERC20" /* STARKDEFI_ERC20 */:
    case "JEDISWAP_ERC20" /* JEDISWAP_ERC20 */: {
      return new Fraction4(memecoin.liquidity.quoteAmount).multiply(new Fraction4(memecoin.launch.teamAllocation, memecoin.totalSupply).add(1)).divide(decimalsScale(memecoin.quoteToken.decimals)).multiply(quoteTokenPriceAtLaunch);
    }
    case "EKUBO_NFT" /* EKUBO_NFT */: {
      const initialPrice = getInitialPrice(memecoin.liquidity.startingTick);
      return new Fraction4(
