...
async function getPairPrice(provider, pair, blockNumber = import_starknet4.BlockTag.latest) {
  if (!pair) return new import_sdk_core3.Fraction(1, 1);
  ...
  const pairPrice = new import_sdk_core3.Fraction(
    import_starknet4.uint256.uint256ToBN({ low: reserve1Low, high: reserve1High }).toString(),
    import_starknet4.uint256.uint256ToBN({ low: reserve0Low, high: reserve0High }).toString()
  );
...
var Factory = class {
  ...
  async getJediswapLiquidityLockPosition(liquidity) {
    ...
    return {
      unlockTime: +result[4],
      owner: (0, import_starknet5.getChecksumAddress)(result[3])
    };
  }
  async getEkuboLiquidityLockPosition(liquidity) {
    ...
    return {
      ...
      owner: (0, import_starknet5.getChecksumAddress)(result[0]),
      poolKey: {
        token0: (0, import_starknet5.getChecksumAddress)(result[2]),
        token1: (0, import_starknet5.getChecksumAddress)(result[3]),
        fee: result[4],
  ...
  async getEkuboFees(memecoin) {
    ...
    const calldata = import_starknet5.CallData.compile([
    ...
    return new import_sdk_core4.Fraction(
      (new import_sdk_core4.Fraction(memecoin.address).lessThan(memecoin.quoteToken.address) ? fees1 : fees0).toString(),
  ...
  getCollectEkuboFeesCalldata(memecoin) {
    ...
    const collectFeesCalldata = import_starknet5.CallData.compile([
      memecoin.liquidity.ekuboId,
      // ekubo pool id
  ...
  getExtendLiquidityLockCalldata(memecoin, seconds) {
    if (!memecoin?.isLaunched || memecoin.liquidity.type === "EKUBO_NFT" /* EKUBO_NFT */) return;
    const extendCalldata = import_starknet5.CallData.compile([
  ...
  getDeployCalldata(data) {
    const salt = import_starknet5.stark.randomAddress();
    const constructorCalldata = import_starknet5.CallData.compile([
      data.owner,
      data.name,
      data.symbol,
      import_starknet5.uint256.bnToUint256(BigInt(data.initialSupply) * BigInt(decimalsScale(DECIMALS))),
      salt
    ]);
    const tokenAddress = import_starknet5.hash.calculateContractAddressFromHash(
      salt,
  ...
  async getEkuboLaunchCalldata(memecoin, data) {
    ...
    const teamAllocationFraction = data.teamAllocations.reduce((acc, { amount }) => acc.add(amount), new import_sdk_core4.Fraction(0));
    const teamAllocationPercentage = new import_sdk_core4.Percent(
      teamAllocationFraction.quotient,
      new import_sdk_core4.Fraction(memecoin.totalSupply, decimalsScale(DECIMALS)).quotient
    );
    const teamAllocationQuoteAmount = new import_sdk_core4.Fraction(data.startingMarketCap).divide(quoteTokenPrice).multiply(teamAllocationPercentage.multiply(data.fees.add(1)));
    const uin256TeamAllocationQuoteAmount = import_starknet5.uint256.bnToUint256(
      BigInt(teamAllocationQuoteAmount.multiply(decimalsScale(data.quoteToken.decimals)).quotient.toString())
    );
    const initialPrice = +new import_sdk_core4.Fraction(data.startingMarketCap).divide(quoteTokenPrice).multiply(decimalsScale(DECIMALS)).divide(new import_sdk_core4.Fraction(memecoin.totalSupply)).toFixed(DECIMALS);
    ...
    const transferCalldata = import_starknet5.CallData.compile([
      FACTORY_ADDRESSES[this.config.chainId],
      // recipient
    ...
    const initialHoldersAmounts = data.teamAllocations.map(
      ({ amount }) => import_starknet5.uint256.bnToUint256(BigInt(amount) * BigInt(decimalsScale(DECIMALS)))
    );
    const launchCalldata = import_starknet5.CallData.compile([
      memecoin.address,
      // memecoin address
      data.antiBotPeriod,
  ...
  async getStandardAMMLaunchCalldata(memecoin, data) {
    ...
    const quoteAmount = new import_sdk_core4.Fraction(data.startingMarketCap).divide(quoteTokenPrice).multiply(new import_sdk_core4.Fraction(1).subtract(teamAllocationPercentage));
    const uin256QuoteAmount = import_starknet5.uint256.bnToUint256(BigInt(quoteAmount.multiply(decimalsScale(18)).quotient.toString()));
    const initialHolders = data.teamAllocations.map(({ address }) => address);
    const initialHoldersAmounts = data.teamAllocations.map(
      ({ amount }) => import_starknet5.uint256.bnToUint256(BigInt(amount) * BigInt(decimalsScale(DECIMALS)))
    );
    const approveCalldata = import_starknet5.CallData.compile([
      FACTORY_ADDRESSES[this.config.chainId],
      // spender
      uin256QuoteAmount
    ]);
...
function getStartingMarketCap(memecoin, quoteTokenPriceAtLaunch) {
  ...
  switch (memecoin.liquidity.type) {
    case "STARKDEFI_ERC20" /* STARKDEFI_ERC20 */:
    case "JEDISWAP_ERC20" /* JEDISWAP_ERC20 */: {
      return new import_sdk_core5.Fraction(memecoin.liquidity.quoteAmount).multiply(new import_sdk_core5.Fraction(memecoin.launch.teamAllocation, memecoin.totalSupply).add(1)).divide(decimalsScale(memecoin.quoteToken.decimals)).multiply(quoteTokenPriceAtLaunch);
    }
    case "EKUBO_NFT" /* EKUBO_NFT */: {
      const initialPrice = getInitialPrice(memecoin.liquidity.startingTick);
      return new import_sdk_core5.Fraction(
